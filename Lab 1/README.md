...
Based on the feedback I collected, I think a good way to incorporate those feedbacks, is to add a voice notification on top of the visual signal( colored lights). The voice notification will tell people briefly what the following events are so that users do not need to memorize the color code they set before. I feel if the calendar just sits in one place, people will likely miss out information. So I decided to make it wearable, like a button on your shirt, or a badge that you can pin on your clothes. In this case, you can take the calendar anywhere you go (indoor). Besides, I think it's very needed to add vibration as part of the notification system because as a wearable device, the vibration can usually catch people's attention quickly (most of the smartwatches uses vibration solely as notification)

For the paper prototype, I used paper to fold a case for my phone, used Figma to remotely control the colored light. Thanks to the actor Chang Han, he mimicked the vibration sound while he was acting. The voice notification was generated by google translator. In the end, I used a clip to attach the entire device to actor Chang's clothes.
