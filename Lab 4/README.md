...
Future implementations of the device might include a game controller instead of individual pads. I would have to perform a few within-group user tests to see which style users preferred. I would also like to implement a drum beat as a backing track so the users have something to play along to so the melody feels more like a song. I would also like to have 8 pads per player in future iterations, so users can have a full scale to play around with. Another QWIIC button will also be used in future implementations to change the scale of the notes on the fly. 
